...
- representative_outputs:
  1) 세특용: (계기→교과/활동→구체적 방법→결과물) 구조. 학생이 '할 수 있는' 수준으로 제안하되, 단순 조사가 아닌 명확한 가설과 탐구 매체(논문/도서/데이터)를 포함하여 3문장 이상 심층적으로 서술.
  2) 창체용: (계기→활동→구체적 방법→결과물) 구조. (위와 동일하게 실험, 통계 분석 등 구체적 탐구 방법론을 3문장 이상 상세히 작성)
...
- subject_table: 과목별 세특 설계. (주력 3개 + 보조 2~3개)
  * 주의: 22개정 교과서에 나온 실제 개념만 활용.
  * 특별 지시: '활동'란 서술 시 1문장 단답형 절대 금지. 반드시 "1) 문제 인식, 2) 탐구 방법론(어떤 문헌/데이터를 쓸지), 3) 도출할 인사이트"를 포함해 3~4문장 분량으로 매우 구체적이고 깊이 있게 작성할 것.
  * 컬럼: 과목 | 교과개념 | 전공연결질문 | 활동(심층 탐구 프로세스) | 결과물(증거)
...
[IMPORTANT] Output strictly in JSON format as defined:
{
  ...
  "page2": {
    "execution_plan": {
      "subject_table": [
        { "subject": "과목명", "concept": "연계 교과 개념", "question": "탐구 질문", "activity": "구체적 심층 탐구 활동 (가설-방법-결론 포함 3~4문장 이상)", "evidence": "증거물" },
        { "subject": "과목명", "concept": "연계 교과 개념", "question": "탐구 질문", "activity": "구체적 심층 탐구 활동 (가설-방법-결론 포함 3~4문장 이상)", "evidence": "증거물" }
      ],
      "creative_experience": {
        "club_options": [
