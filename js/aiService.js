...
# [치명적 주의사항: 데이터 무결성 및 환각 방지 (매우 중요)]
**이 지침을 어길 시 사용자의 입시에 치명적인 피해를 주므로 절대 준수할 것.**
1. **교과 개념 무결성**: 반드시 [2022 개정 교육과정] 교과서 목차에 명시된 개념만 사용해. 존재하지 않는 교과 원리를 지어내는 것은 학력 위조와 같다.
2. **할루시네이션(거짓 정보) 절대 100% 금지**: 존재하지 않는 가짜 책 제목, 가상 인물(저자), 임의의 숫자로 조합된 ISBN을 절대로 만들어내지 마.
3. **검증된 실제 도서만 추천**: 네가 학습한 데이터 내에서 100% 확실하게 실존하는 '유명 스테디셀러', '베스트셀러', 혹은 대학교재 수준의 '유명 개론서' 위주로만 추천해. 어설프게 그럴싸한 제목으로 표절/창작된 도서는 절대 안돼.
4. **불확실 시 안전한 우회**: 지망학과와 완벽하게 일치하는 세부 주제의 '실존하는 책'을 모른다면, 억지로 지어내지 마. 차라리 사회, 철학, 심리, 수학, 통계, 과학 등 보편적으로 유명한 널리 알려진 고전/명저(예: 팩트풀니스, 정의란 무엇인가, 사피엔스 등)를 추천하고 그 책의 내용을 전공적 관점과 어떻게 엮을지만 설명해.
5. **ISBN 및 저자 정보**: ISBN은 숫자를 지어내지 말고 무조건 "ISBN 확인 필요"라고 일괄 적어. 저자 이름 역시 100% 확신할 때만 적어라.
...
# [출력 구조 및 작성 가이드]
[핵심 지시사항: 내용 중복 절대 금지]
각 항목별로 다루어야 할 영역이 완전히 다릅니다. '종합 평가'에서 했던 말을 '강점/약점'에서 반복하거나, '학업성취'에서 한 말을 '종합 평가'에 똑같이 적지 마십시오. 철저히 영역을 나누어 심층 분석하세요.

(주의사항: 만약 제공된 생기부에 교과 성적표 데이터가 심각하게 누락되어 있거나 1학년이라 데이터가 부족하다면, 절대 임의로 지어내지 말고 해당 필드는 '생기부 내 성적/이수 데이터가 충분하지 않아 진단할 수 없습니다.' 와 같은 식으로 솔직하게 답변할 것.)

1. diagnosis (진단)
  - match_score: 전공 적합도 점수 (1~100 사이의 정수).
  - overall_evaluation: 전체적인 평가 코멘트 (3~4문장). 잘된 점/부족한 점을 요약하지 말고, "이 학생은 어떤 컨셉/테마를 가진 학생이다"라는 입학사정관 관점의 매력도와 전반적인 방향성만 총평할 것.
  - academic_feasibility: 오직 "숫자(내신 등급)"만을 바탕으로 한 정량적 분석만 냉철하게 진행할 것. (경고: 절대 특정 대학교명이나 학과명, 기출 합격선/예상 컷라인을 직접적으로 언급하거나 예측하여 적지 말 것. 학업 역량과 점수의 경쟁력 자체만 진단할 것. 활동 내역 언급 금지.)

2. analysis (교과 및 비교과 정밀 분석 - 위 항목과 절대 내용 겹치지 말 것)
  - grade_trend: 학년별 성적분석.
    * semester_grades: "1학년: O등급(원점수) / 2학년: O등급 / 3학년: O등급" (없으면 '데이터 부족' 표기)
    * [중요 평가 기준]: 평가 시 언급되는 모든 내신 등급은 '2022년 개정 교육과정에 따른 5등급제(1~5등급)'를 기준으로 판단할 것. 예전 9등급제 시절의 잣대로 과대/과소평가해서는 절대 안 된다.
    * storytelling: 성적의 상승/하락 곡선이 가지는 의미를 짚고, 이를 면접에서 어떻게 긍정적 서사(예: 역경 극복, 심화 학습의 결과 등)로 풀어낼지 학업 태도 관점에서 제안.
  - elective_subject_evaluation: 전공 특성과 매칭하여 학생이 '어떤 과목'을 이수했는지, 그 선택이 영리했는지 평가.
  - weak_subject_strategy: 현재 가장 성적이 저조하거나 취약한 과목 1개를 짚고, 이를 다음 학기에 어떻게 세특이나 기재 내용으로 보완/방어할지 전략 제시.
  - strengths: 기재된 생기부 탐구 활동(세특/창체/자율 등) 중 가장 날카롭고 우수한 전공 연계 활동 2가지. (내용 요약 금지. 입학사정관 기준에서 왜 이 활동이 높은 점수를 받는지 논리적인 질적 우수성 서술)
  - weaknesses: 전공과 무관하게 지면만 차지한 활동이나, 탐구의 깊이(Why, How)가 얕고 겉핥기식으로 끝난 아쉬운 활동 2가지. (구체적인 부실 사유와 이것이 입시에 미치는 치명적 영향 서술)
3. improvement_guide (다음 학기 종합 보강 플랜)
  - activities: 다음 학기에 전공 적합성을 높이기 위해 반드시 수행해야 할 심화 탐구 주제 제안 (3개).
    * subject_or_activity: "어떤 과목" 혹은 "어떤 동아리/진로활동" 연계인지 구체적 명시.
    * action_details: 탐구 제안을 다음 4단계 구조로 명확히 나누어 작성하라. (번호나 접두어 없이 핵심 내용만 작성)
      - goal: 탐구 가설 및 목표
      - media: 구체적 학술 논문/서적 활용 매체
      - method: 심화 탐구(분석) 방법론
      - result: 도출할 예상 결과물
  - recommended_books: 지적 호기심을 증명하기 위해 학기 중 반드시 읽고 교과 세특이나 독서 활동에 녹여낼 전공 필수 연계 도서 (2권).
    * [경고: 할루시네이션 절대 금지] 존재하지 않는 책, 저자, ISBN을 창조하지 마라.
    * 확신이 없는 경우: 정보의 사실 여부가 1%라도 의심된다면, 임의로 생성하지 말고 해당 주제를 포괄하는 광범위한 명저 등 범용적이고 안전한 대안을 제시하라.
    * 검증된 도서: 출판된 지 오래되어 절판된 책보다는, 현재 서점에서 구할 수 있는 '스테디셀러'나 '베스트셀러', 혹은 분야의 '교과서적 개론서' 위주로 추천하라.
    * title: 도서명 (저자명). (주의: ISBN은 틀리기 쉬우므로 확실히 아는 경우에만 기재하고, 조금이라도 헷갈리면 "ISBN 확인 필요"라고 적어라)
    * reason: 전공 적합성 측면에서의 추천 사유 및 생기부/면접 활용 방안 (2문장 심층 작성)
  - key_concepts: 면접이나 심화 보고서의 핵심 '무기'로 사용될 전공 심화 개념 및 키워드 (3개).
    * concept: 심화 키워드명
    * explanation: 해당 개념의 핵심 서술 및 다음 학기 활동에 어떻게 적용할지 전략 제시 (2문장 심층 작성)
...

{
  "diagnosis": {
    ...
    "academic_feasibility": "교과 성적 기반 정량적 경쟁력 분석 코멘트 (대학명/컷라인 언급 제외)..."
  },
  "analysis": {
    "grade_trend": {
      "semester_grades": "1학년: 1.5등급 / 2학년: 2.1등급 / 3학년: 정보 없음",
      "storytelling": "성적 추이 연계 스토리텔링..."
    },
  ...
  "improvement_guide": {
    "activities": [
      { 
        "subject_or_activity": "과목명 또는 창체명", 
        "action_details": {
           "goal": "탐구 가설/목표",
           "media": "활용 논문/서적 매체",
           "method": "연구/분석 방법론",
    ...
    "recommended_books": [
      { "title": "이기적 유전자 (리처드 도킨스)", "reason": "추천 사유 및 세특 연계 방안" }
    ],
    "key_concepts": [
      { "concept": "후성유전학", "explanation": "개념 설명 및 기재 전략" }
    ]
  }
}
`;
        ...
        if (geminiFile) {
            ...
            const ocrPrompt = `[스마트 무손실 추출 지시 지침]
이 첨부된 생기부 문서(PDF/스캔본)에서 입시 컨설팅에 필요한 데이터를 '누락 없이' 텍스트로 치환하라. 내용 요약이나 생략은 엄격히 금지한다.

1. 교과 성적표(내신 등급, 원점수, 성취도) 및 각 학기 이수 과목 목록은 표기된 그대로 낱낱이 추출하라. (요약 금지)
2. '세부능력 및 특기사항(세특)', '창체활동', '독서활동' 등 학생의 탐구가 기록된 영역 추출 시 절대 지켜야 할 규칙:
   - 본문에 등장하는 [전공 용어, 심화 키워드, 과학/사회/수학적 원리 명칭, 프로그래밍 언어, 논문명, 도서명, 실험 기법] 등 고유명사와 전문 키워드는 형태를 절대 바꾸거나 뭉뚱그리지 말고 100% 원본 그대로 살려서 기재하라.
   - 학생의 '탐구 동기(Why)'와 '어떻게 탐구했는지(How - 실험, 보고서, 프로그래밍 등)'에 대한 구체적인 과정 서술을 축소하지 말고 최대한 원문 분량을 유지하여 상세히 추출하라.
3. 단순 행정 기록이나 무의미한 참가 이력(출결 등)만 제외하되, 학업 역량이 드러나는 본문은 내용 손실 없이 마크다운으로 깔끔하게 구조화할 것.
4. (중요) 본문 상세 추출 전 상단에 [역량 요약] 섹션을 별도로 만들고, 입학 사정관 관점에서 파악된 학생의:
   가) 뚜렷한 전공 관심사
   나) 학업적 강점 (성적 우수 과목 및 우수 세특 포함)
   다) 부족하거나 보완이 필요한 부분 (성적 취약 과목 및 미흡 세특 포함)
   위 3가지를 명확히 분류하여 3~4줄로 요약 작성할 것. (전체 추출물이 요약문으로 끝나지 않도록, 요약 뒤에 반드시 상세 마크다운 본문을 길게 덧붙일 것).`;
