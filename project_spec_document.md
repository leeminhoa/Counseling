...
## 6. 최근 업데이트 내역 (V3.0+)
- **생기부 OCR 무손실 추출 (aiService.js)**: 
  - 서류 요약 과정에서 발생하던 전문 용어(전공 개념, 도서명, 실험명) 누락을 막기 위해 프롬프트 지시어를 '요약'에서 '무손실 100% 보존 추출'로 전면 개편.
- **AI 토큰 사용량 관측 (aiService.js)**: 
  - '생기부 첨삭' 및 '탐구가이드' 생성 시, 사용된 Input(Prompt) 및 Output(Candidates) 토큰을 브라우저 개발자 도구 콘솔에 실시간 누적 출력하도록 모니터링 로직 추가.
- **LLM 모델 강제성 보장 (aiService.js)**: 
  - 브라우저 로컬스토리지에 기존 모델 캐시 데이터가 꼬여있어 구버전 LLM이 호출되는 것을 막고자, 모든 생성 호출부 초입에 모델명을 `gemini-3.1-pro-preview`로 덮어쓰도록 강제(Auto-Fix) 방어 로직 추가.
- **안정적 신규 학생 등록 및 목록 동기화 (profile.js, manager.js)**: 
  - 신규 학생 등록 모달 진입 시 기존 데이터 컨텍스트 초기화.
  - 신규 등록 완료 후 수파베이스 DB 저장 응답(Async Await)을 완전히 수신받은 뒤 학생 목록 UI를 새로고침하여 경합 조건(Race Condition)으로 인한 비정상 갱신 현상 완벽 해결.
