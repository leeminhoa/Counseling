...
async function run() {
  const { data: sData } = await supabase.from('subjects').select('*').limit(1);
  console.log("subjects sample:", sData);
  
  const { data: mData } = await supabase.from('v_univ_dept_subjects').select('*').limit(1);
  console.log("v_univ_dept_subjects sample:", JSON.stringify(mData, null, 2));
}
run();
